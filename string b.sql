...
Q10 select lower([first_name])first_name, 
    substring(lower([first_name]), 0,2)+lower([last_name])
	+'@systemcos.com'as email_address
    from[dbo].[employees] 
...
Q13 select [first_name],[salary]from [dbo].[employees]
    WHERE salary not IN  ( SELECT MIN(salary)  
    FROM employees) 

Q14 select [first_name],[salary]from [dbo].[employees]
